...
class Obmp_review extends CI_Controller {
    ...
    function __construct() {
      ...
      //  $idiom = 'english';
        $this->idiom = $idiom = $this->uri->segment(5, 'italian' ); 
...
//         $hotel_id = $this->uri->segment(3, 2 );
    ...
    //  print_r( $data['review']); 
        ...
        $this->form_validation->set_rules('postazione_id', 'lang:postazione_id', 'required|trim');
        ...
        if ($this->form_validation->run() == FALSE) { // validation hasn't been passed
            ...
            if ($this->obmp_review_model->insert($form_data) == TRUE) { // the information has therefore been successfully saved in the db
                redirect('obmp_review/?' . $_SERVER['QUERY_STRING']);   // or whatever logic needs to occur
            } else {

                redirect('obmp_review/?error=noinsert&' . $_SERVER['QUERY_STRING']);
            }
        }
    }
    ...
    function edit() {
        ...
        if ($this->form_validation->run() == FALSE) { // validation hasn't been passed
            ...
            if ($this->obmp_review_model->update($review_id, $form_data) == TRUE) { // the information has therefore been successfully saved in the db
                ...
                redirect('obmp_review/index/?error=noupdata&' . $_SERVER['QUERY_STRING']);
            }
        }
    }
