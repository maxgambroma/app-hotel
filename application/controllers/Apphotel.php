...
// 
        $data['hotel_id'] = $hotel_id;
        $data['rooms'] = $this->camere_model->camere($hotel_id);
        $data['foglio'] = $this->camere_model->foglio_dettagli($hotel_id, $today);
...
// carica la vista del contenuto
        $vista = 'foglio';
// gestore templete
        $data['temp'] = array('templete' => $temi, 'contenuto' => $vista, 'bar1' => 'bar_1', 'bar_2' => 'bar_foglio');
        $this->load->view('templetes', $data);
// fine
    }
...
//                'pulizia_data_record' => set_value('pulizia_data_record'
        ...
        if ($this->pulizia_model->SaveForm($form_pulizia) == TRUE) { // the information has therefore been successfully saved in the db
            redirect(base_url() . '/index.php/apphotel/?scelta=controllo&hotel_id=' . $this->input->post('hotel_id') . '');   // or whatever logic needs to occur
        }
    }
    ...
    function manutenzioni() {
        ...
        if ($this->guasti_model->SaveForm($form_guasti) == TRUE) { // the information has therefore been successfully saved in the db
            if ($this->input->post('conto_id')) {


                redirect(base_url() . 'index.php/apphotel/risorsa?conto_id=' . $this->input->post('conto_id') . 'foglio_id=' . $this->input->post('foglio_id') . '&hotel_id=' . $this->input->post('hotel_id') . '');
            }
            if ($this->input->post('camera_id') && !$this->input->post('conto_id')) {

//                nel caso non ci siano conti aperti
//                index.php/risorsa/camera/?camera_id=53&hotel_id=4

                redirect(base_url() . 'index.php/risorsa/camera/?camera_id=' . $this->input->post('camera_id') . '&hotel_id=' . $this->input->post('hotel_id') . '');
            }


            if ($this->input->post('camera_id') == 0 && !$this->input->post('conto_id')) {
